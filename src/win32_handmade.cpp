...
CompleteAndGetNextWorkQueueEntry(work_queue *Queue, work_queue_entry Completed)
{
	...
	if(Completed.IsValid) InterlockedIncrement((LONG volatile *)&Queue->EntryCompletionCount);
...
ThreadProc(LPVOID lpParameter)
{
	...
	for(;;)
	{
		...
		if(Entry.IsValid) DoWorkerWork(Entry, ThreadInfo->LogicalThreadIndex);
		else WaitForSingleObjectEx(ThreadInfo->Queue->SemaphoreHandle, INFINITE, FALSE);
	}
